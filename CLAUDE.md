...
- **equipos** (`EquiposService`, `Equipo` interface) — teams, grouped into groups A–L, with classification-stage fields (`e32`, `of`, `cf`, `sf`, `gf` representing Eliminatoria 32 → Octavos → Cuartos → Semifinal → Final) and score fields (`pg`, `pe`, `pp`, `p32`, `po`, `pc`, `ps`, `pf`). `EquiposService.cargarEquiposEspeciales()` builds lookup maps (e.g. `unoA`/`dosA` for group winners/runners-up, `o1..o16`, `c1..c8`, `s1..s4`, `f1..f2`) used to render the knockout bracket in `fixture`/`clasificacion`.
...
- **juegos** (`JuegosService`, `Juego` interface) — scheduled/played matches (local/visitante teams, fase, fecha, hora, scores).

Classification fields (`e32`/`of`/`cf`/`sf`/`gf`) have a dependency chain enforced in `EquiposService` (`validateClasificacionField`, `cleanDependentClasificacionFields`): a team can't be assigned to a later stage without the prior stage set, and changing an earlier stage clears all later ones.

### Routing & auth

Routes are defined in `src/app/app.routes.ts`. Admin/management routes (`admin`, `puntajes`, `usuarios`, `ingresar-juego`, `participantes`, `asignacion`, `clasificacion`) are protected by `authGuard`; `login` is protected by `guestGuard` (redirects away if already authenticated). Public routes (`tabla-puntajes`, `equipos`, `juegos`, `reglamento`, `fixture`) require no auth.
